...
async fn main() -> anyhow::Result<()> {
    ...
    for (_, map) in &maps {
        let map_records = sqlx::query_as!(Record, "SELECT * from records WHERE map_id = ? ORDER BY time", map.id).fetch_all(&mysql_pool).await?;

        // Skip maps without records
        if map_records.is_empty() { continue; }
    ...
    for (map_id, score) in &map_scores
    {
        ...
        write!(&mut map_ladder, "{},{},{},{},{},{},{},{},{}\n", map_id, map.name, score, average, stats.min_record, stats.max_record, stats.average_record, stats.median_record, stats.records_count)?;
    }

    println!("Computed score for {} players and {} maps.", player_scores.len(), map_scores.len());
